...
CREATE VIEW [average_ride_duration_monthyear2]
AS
SELECT MonthYear
	,member_casual
...
CREATE VIEW [Bike_Preference_by_months]
AS
SELECT month
	,rideable_type
	,COUNT(ride_id) AS Total_Rides
...
CREATE VIEW [TotalRides_by_month]
AS
SELECT month
	,member_casual
	,COUNT(ride_id) AS Total_rides
...
CREATE VIEW [Total_Rides_by_start_hour]
AS
